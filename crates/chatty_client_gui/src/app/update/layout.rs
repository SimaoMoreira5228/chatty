...
impl Chatty {
	pub fn update_export_layout_pressed(&mut self) -> Task<Message> {
		let root = crate::app::features::layout::UiRootState::from_app(self);
		self.state.ui.active_overlay = Some(crate::app::features::overlays::ActiveOverlay::Layout(
			LayoutModal::new_export(root, None),
		));
		Task::none()
	}

	pub fn update_reset_layout_pressed(&mut self) -> Task<Message> {
		self.state.ui.active_overlay = Some(crate::app::features::overlays::ActiveOverlay::Layout(LayoutModal::new_reset()));
		Task::none()
	}

	pub fn update_confirm_export(&mut self) -> Task<Message> {
		if let Some(crate::app::features::overlays::ActiveOverlay::Layout(modal)) = &self.state.ui.active_overlay
			&& let LayoutModalKind::Export { root, path } = &modal.kind
			&& let Some(path) = path
		{
			let root = root.clone();
			let path = path.clone();
			...
			return Task::perform(
				...
				move |_| Message::AnimationTick(tick), // Dummy message to satisfy Task::perform
			);
		}
		Task::none()
	}
	...
	pub fn update_confirm_reset(&mut self) -> Task<Message> {
		let root = crate::app::features::layout::UiRootState::default();
	...
	pub fn update_choose_export_path_pressed(&mut self) -> Task<Message> {
		Task::perform(
			...
			|res| Message::OverlayMessage(OverlayMessage::Layout(LayoutModalMessage::ChooseExportPathResult(res))),
		)
	}

	pub fn update_layout_export_path_chosen(&mut self, path: Option<PathBuf>) -> Task<Message> {
		if let Some(crate::app::features::overlays::ActiveOverlay::Layout(modal)) = &mut self.state.ui.active_overlay
			&& let LayoutModalKind::Export { path: p, .. } = &mut modal.kind
		{
			*p = path;
		}
		Task::none()
		...
		Task::perform(
			async {
				...
				if let Some(handle) = path {
					...
					match serde_json::from_slice::<crate::app::features::layout::UiRootState>(&bytes) {
						Ok(root) => Ok(root),
			...
			|res| Message::OverlayMessage(OverlayMessage::Layout(LayoutModalMessage::LayoutImportFileParsed(res))),
		)
	}
	...
	pub fn update_confirm_import(&mut self) -> Task<Message> {
		if let Some(crate::app::features::overlays::ActiveOverlay::Layout(modal)) = &self.state.ui.active_overlay
			&& let LayoutModalKind::Import { root } = &modal.kind
		{
			let root = root.clone();
			self.apply_ui_root(root.clone());
			self.save_ui_layout();
	...
	pub fn update_modal_dismissed(&mut self) -> Task<Message> {
		...
		if let Some(crate::app::features::overlays::ActiveOverlay::Layout(modal)) = &self.state.ui.active_overlay
			&& let LayoutModalKind::Import { .. } = &modal.kind
		{
