...
use crate::app::types::{PlatformChoice, SplitLayoutChoice, ThemeChoice};
use crate::theme;
...
pub fn view(app: &Chatty, palette: theme::Palette) -> Element<'_, Message> {
	...
	let theme_picker = pick_list(all_themes, Some(current_theme), |c| {
		Message::Settings(SettingsMessage::ThemeSelected(c))
	});
	...
	let split_picker = pick_list(&SplitLayoutChoice::ALL[..], Some(current_split), |c| {
		Message::Settings(SettingsMessage::SplitLayoutSelected(c))
	});

	let platform_picker = pick_list(
		&PlatformChoice::ALL[..],
		Some(PlatformChoice(app.state.gui_settings().default_platform)),
		|c| Message::Settings(SettingsMessage::PlatformSelected(c)),
	);
		...
		column![
			...
			row![
				...
				text_input("2000", &app.state.ui.max_log_items_raw)
					.on_input(|v| Message::Settings(SettingsMessage::MaxLogItemsChanged(v))),
			]
			.spacing(12)
			.align_y(Alignment::Center),
			row![
				...
				iced::widget::checkbox(gs.auto_connect_on_startup)
					.on_toggle(|v| Message::Settings(SettingsMessage::AutoConnectToggled(v))),
			]
			.spacing(12)
			.align_y(Alignment::Center),
			row![text(t!("settings.locale")).color(palette.text_dim), {
				...
				pick_list(locales, selected, |s: &'static str| {
					Message::Settings(SettingsMessage::LocaleSelected(s.to_string()))
				})
			}]
			.spacing(12)
			.align_y(Alignment::Center),
