...
impl SettingsView {
	...
	pub fn update(&mut self, app: &mut Chatty, message: SettingsMessage) -> Task<Message> {
		match message {
			...
			SettingsMessage::IdentityUse(id) => app.update_identity_use(id),
			SettingsMessage::IdentityToggle(id) => app.update_identity_toggle(id),
			SettingsMessage::IdentityRemove(id) => app.update_identity_remove(id),
			SettingsMessage::ClearIdentity => app.update_clear_identity(),
