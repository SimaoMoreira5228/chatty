...
use crate::app::{Chatty, Message, ShortcutKeyChoice};
use crate::theme;
...
pub fn view(app: &Chatty, palette: theme::Palette) -> Element<'_, Message> {
	...
	let drag_modifier_picker = pick_list(
		...
		|c| Message::SettingsMessage(SettingsMessage::DragModifierSelected(c)),
	);

	let close_key_input = text_input("q", &keybinds.close_key)
		.on_input(|s: String| Message::SettingsMessage(SettingsMessage::CloseKeyChanged(single_char(&s))))
		.width(Length::FillPortion(1));
	let new_key_input = text_input("n", &keybinds.new_key)
		.on_input(|s: String| Message::SettingsMessage(SettingsMessage::NewKeyChanged(single_char(&s))))
		.width(Length::FillPortion(1));
	let reconnect_key_input = text_input("r", &keybinds.reconnect_key)
		.on_input(|s: String| Message::SettingsMessage(SettingsMessage::ReconnectKeyChanged(single_char(&s))))
		.width(Length::FillPortion(1));
	...
	scrollable(
		column![
			...
			row![
				...
				iced::widget::checkbox(keybinds.vim_nav)
					.on_toggle(|v| Message::SettingsMessage(SettingsMessage::VimNavToggled(v)))
			]
			.spacing(12)
			.align_y(Alignment::Center),
			row![
				...
				container(
					text_input("h", &keybinds.vim_left_key)
						.on_input(|s: String| Message::SettingsMessage(SettingsMessage::VimLeftKeyChanged(single_char(&s))))
						.width(Length::FillPortion(1))
				)
				.padding(2)
				.style(move |_theme| container::Style {
			...
			row![
				...
				container(
					text_input("j", &keybinds.vim_down_key)
						.on_input(|s: String| Message::SettingsMessage(SettingsMessage::VimDownKeyChanged(single_char(&s))))
						.width(Length::FillPortion(1))
				)
				.padding(2)
				.style(move |_theme| container::Style {
			...
			row![
				...
				container(
					text_input("k", &keybinds.vim_up_key)
						.on_input(|s: String| Message::SettingsMessage(SettingsMessage::VimUpKeyChanged(single_char(&s))))
						.width(Length::FillPortion(1))
				)
				.padding(2)
				.style(move |_theme| container::Style {
			...
			row![
				...
				container(
					text_input("l", &keybinds.vim_right_key)
						.on_input(|s: String| Message::SettingsMessage(SettingsMessage::VimRightKeyChanged(single_char(&s))))
						.width(Length::FillPortion(1))
				)
				.padding(2)
				.style(move |_theme| container::Style {
			...
			row![
				button(text(t!("export_layout_title")))
					.on_press(Message::SettingsMessage(SettingsMessage::ExportLayoutPressed)),
				button(text(t!("settings.import_layout_clipboard")))
					.on_press(Message::SettingsMessage(SettingsMessage::ImportLayoutPressed)),
				button(text(t!("settings.import_from_file")))
					.on_press(Message::SettingsMessage(SettingsMessage::ImportFromFilePressed)),
				button(text(t!("settings.reset_layout")))
					.on_press(Message::SettingsMessage(SettingsMessage::ResetLayoutPressed)),
			]
			.spacing(8)
			.align_y(Alignment::Center),
